from states_lite import CollapsedState, SuperState, Teacher, Subject, Section, Table_T, GroupID_T, Group_T
...
def print_to_csv(tt, dims, elective_slots, blocked_slots, faculty:list[Teacher]):
    ...
    with open('timetable_student_lite.csv', 'w') as file:
    ...
    with open('timetable_faculty_lite.csv', 'w') as file:
        for fac in faculty:
...
def setup(dims5,
    ...
    subjects_dict:dict[str, Subject], faculty_subj_sec_dict:dict[str, tuple[str,int]],
    ...
    # removes states where the teacher is not assigned to
    for i in range(n_sections):
        invalidIDs = set()
        for fac in teachers:
            invalidIDs |= fac.groupIDs
            for subj_nm, sec in faculty_subj_sec_dict[fac.name]:
                if subj_nm in ['GTA', 'CNS', 'DA', 'BD', 'IOT', 'ARVR', 'AC', 'HCI']:
                    continue
                if sec != i: continue                
                invalidIDs -= {(fac.id, subj.id) for subj in subjects_dict[subj_nm]}
        for j in range(n_days_per_week):
            for k in range(n_slots_per_day):
                if isinstance(table[i][j][k], CollapsedState):continue
                table[i][j][k].classes -= invalidIDs
...
def input_file(filename):
    with open(filename, 'r') as file:
        ...
        #faculty
        faculty_dict = {}
        faculty_subj_sec_dict:dict[str, tuple[str,int]] = {}
        for _ in range(n_faculty):
            faculty, subjs = map(str.strip, file.readline().split('='))
            subj_list = []
            subj_sec_list = []
            for subj in subjs.split(','):
                print(f"{subj=} {subjs=}: {subj.strip().split(' ')=}")
                subj_nm, sec_no = subj.strip().strip("()").split(' ')
                subj_list.append(subj_nm)
                subj_sec_list.append((subj_nm, int(sec_no)))
            faculty_dict[faculty] = subj_list
            faculty_subj_sec_dict[faculty] = subj_sec_list
        ...
        for i in range(n_semesters):
            ...
            semester_electives_list.append(
                [] if elecs.strip() == "0" else [*map(lambda x:int(x)-1, elecs.split(','))]
            )
    ...
    for k,v in faculty_dict.items():
        print(k,v)
    print("faculty section subjects")
    for k,v in faculty_subj_sec_dict.items():
        print(k,v)
    ...
    for i,elecs in enumerate(semester_electives_list):
        print(f"Semester{i+1}: ({len(elecs)} electives)", ', '.join(map(lambda x:f"e{x+1}",elecs)))
    print("Blocked slots")
    for i in range(n_semesters):

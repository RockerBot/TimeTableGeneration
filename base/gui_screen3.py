...
class TeacherScreen(Screen):
    ...
    def set_data(self, subjects, teachers, n_electives, n_sections):
        self.n_teachers = len(teachers)
        self.n_sections = n_sections
        subjs = [*subjects]

        self.layout_teachers.clear_widgets()
        for sem_i in range(len(n_sections)):
            list_entry = ListEntry(f'sem {sem_i+1}', subjs, spacing=10, size_hint_y=None, height=dp(30))
            self.layout_teachers.add_widget(list_entry)

        self.layout_teachers.add_widget(Label(text="", size_hint_y=None, height=dp(30)))
    ...
    def goto_next_screen(self, instance):
        ...
        for i, child in enumerate(self.layout_teachers.children):
            if i < self.n_teachers:
                teachers[child.get_text()] = child.get_subjects()
            elif isinstance(child, Label): continue
            else:
                print(child.get_text(),child.get_subjects())
                semester_subjects_list.insert(0, child.get_subjects())
        print('teachers', teachers)
        print('semester_subjects_list', semester_subjects_list)
        self.pass_data(teachers, semester_subjects_list)
        self.manager.current = 'availability'

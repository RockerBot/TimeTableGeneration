...
class InputScreen(Screen):
    def __init__(self, take_input, **kwargs):
        ...
        self.input_sec = TextInput(hint_text='Enter number of sections per semester', size_hint_y=None, height=dp(50))
    ...
    def goto_next_screen(self, instance):
        ...
        n_sections = self.input_sec.text.strip() and tuple(map(int,self.input_sec.text.strip().split(','))) or (5, 5)
        ...
        n_semesters, n_days, n_slots, n_electives = len(n_sections), int(n_days), int(n_slots), int(n_electives)
